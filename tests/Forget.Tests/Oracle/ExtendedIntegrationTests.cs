...
namespace Forget.Tests.Oracle
{
    ...
    public class ExtendedIntegrationTests
    {
        ...
        public ExtendedIntegrationTests(OracleFixture fixture)
        {
            _fixture = fixture;
        }
        ...
        {
            for (int i = 0; i < 3; i++)
                await _fixture.Connection.InsertAsync(new Widget { Id = 200 + i, Name = $"First{i}", IsActive = 1, Price = 1m }, cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 100, Name = "First candidate", IsActive = 1, Price = 1m }, cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 101, Name = "Unique", IsActive = 1, Price = 1m }, cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 210, Name = "Twin", IsActive = 1, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 211, Name = "Twin", IsActive = 1, Price = 1m }, cancellationToken: Ct);
        ...
        {
            for (int i = 0; i < 5; i++)
                await _fixture.Connection.InsertAsync(new Widget { Id = 110 + i, Name = $"Page{i}", IsActive = 1, Price = 1m }, cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 130, Name = "Old130", IsActive = 0, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 131, Name = "Old131", IsActive = 0, Price = 1m }, cancellationToken: Ct);

            Widget[] updates =
            [
                new() { Id = 130, Name = "New130", IsActive = 1, Price = 2m },
                new() { Id = 131, Name = "New131", IsActive = 1, Price = 2m },
            ];
        ...
        public async Task DeleteRangeAsync_RemovesEveryEntityInTheBatch()
        {
            Widget[] toDelete =
            [
                new() { Id = 140, Name = "D140", IsActive = 1, Price = 1m },
                new() { Id = 141, Name = "D141", IsActive = 1, Price = 1m },
            ];
        ...
        {
            await _fixture.Connection.InsertRangeAsync([new Widget { Id = 220, Name = "D220", IsActive = 1, Price = 1m }, new Widget { Id = 221, Name = "D221", IsActive = 1, Price = 1m }, new Widget { Id = 222, Name = "D222", IsActive = 1, Price = 1m }], cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 150, Name = "Exists150", IsActive = 1, Price = 1m }, cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 160, Name = "Count160", IsActive = 1, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 161, Name = "Count161", IsActive = 0, Price = 1m }, cancellationToken: Ct);

            long activeCount = await _fixture.Connection.CountAsync<Widget>(w => w.IsActive == 1 && (w.Id == 160 || w.Id == 161), cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 170, Name = "Agg170", IsActive = 1, Price = 10m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 171, Name = "Agg171", IsActive = 1, Price = 20m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 172, Name = "Agg172", IsActive = 1, Price = 30m }, cancellationToken: Ct);
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 230, Name = "Avg230", IsActive = 1, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 231, Name = "Avg231", IsActive = 1, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 232, Name = "Avg232", IsActive = 1, Price = 2m }, cancellationToken: Ct);
        ...
        [Fact]
        public async Task AvgAsync_ReturnsTheSameAverageWhenTheSessionUsesACommaAsDecimalSeparator()
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 240, Name = "Nls240", IsActive = 1, Price = 10m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 241, Name = "Nls241", IsActive = 1, Price = 15m }, cancellationToken: Ct);

            // The average is read as text and parsed, so the text must not depend on the session's NLS settings.
            await SetNumericCharactersAsync(",.");

            try
            {
                decimal? avg = await _fixture.Connection.AvgAsync<Widget>(w => w.Price, w => w.Id >= 240 && w.Id <= 241, cancellationToken: Ct);

                Assert.Equal(12.5m, avg);
            }
            finally
            {
                await SetNumericCharactersAsync(".,");
        ...
        private async Task SetNumericCharactersAsync(string characters)
        {
            await using OracleCommand command = _fixture.Connection.CreateCommand();
            command.CommandText = $"ALTER SESSION SET NLS_NUMERIC_CHARACTERS = '{characters}'";
        ...
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 180, Name = "Bulk180", IsActive = 0, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 181, Name = "Bulk181", IsActive = 0, Price = 1m }, cancellationToken: Ct);

            int affected = await _fixture.Connection.UpdateAsync<Widget>(new { IsActive = 1 }, w => w.Id == 180, cancellationToken: Ct);
            ...
            Assert.Equal(1, w180!.IsActive);
            Assert.Equal(0, w181!.IsActive);
        }

        [Fact]
        public async Task DeleteAsync_WithPredicate_DeletesOnlyMatchingRows()
        {
            await _fixture.Connection.InsertAsync(new Widget { Id = 190, Name = "DelPred190", IsActive = 1, Price = 1m }, cancellationToken: Ct);
            await _fixture.Connection.InsertAsync(new Widget { Id = 191, Name = "DelPred191", IsActive = 1, Price = 1m }, cancellationToken: Ct);
