...
namespace Forget.Tests.Oracle
{
    ...
    public class SyncIntegrationTests
    {
        ...
        public SyncIntegrationTests(OracleFixture fixture)
        {
            _fixture = fixture;
        }

        private static SyncWidget NewWidget(int id, string name = "sync", decimal price = 1m) => new() { Id = id, Name = name, IsActive = 1, Price = price };
        ...
        {
            _fixture.Connection.InsertRange([.. Enumerable.Range(0, 4).Select(i => new SyncWidget { Id = 350 + i, Name = "Bulk", IsActive = 0, Price = 1m })]);

            Assert.Equal(1, _fixture.Connection.Update<SyncWidget>(new { IsActive = 1 }, w => w.Id == 350));
            Assert.Equal(1, _fixture.Connection.Update<SyncWidget>(new { IsActive = 1 }, new FilterDescriptor<SyncWidget>("Id", 351)));
            Assert.Equal(1, _fixture.Connection.GetById<SyncWidget>(350)!.IsActive);
            Assert.Equal(1, _fixture.Connection.GetById<SyncWidget>(351)!.IsActive);
            Assert.Equal(0, _fixture.Connection.GetById<SyncWidget>(352)!.IsActive);
        ...
        {
            using OracleConnection connection = new(_fixture.ConnectionString);
